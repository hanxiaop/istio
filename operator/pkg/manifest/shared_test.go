...
func Test_mapHelmToIstioOperator(t *testing.T) {
	values := `# The ambient profile enables ambient mode. The Istiod, CNI, and ztunnel charts must be deployed
meshConfig:
  defaultConfig:
    proxyMetadata:
...
cni:
  ...
  excludeNamespaces:
    - kube-system
`
	v, err := convertToInternalIOP(values)
	if err != nil {
		t.Fatalf("mapHelmToIstioOperator() error = %v", err)
	}
	if v == "" {
		t.Fatalf("mapHelmToIstioOperator() returned nil")
	}
}
